...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run_p.add_argument(
        "--workspace", default=".", help="Sandbox root for tool use (default: cwd)."
    )
    run_p.add_argument(
        "--mock", action="store_true", help="Use the offline mock LLM instead of the API."
    )
    ...
    run_p.add_argument("--system", default=None, help="Override the system prompt text.")
    run_p.add_argument(
        "--system-file", default=None, help="Load system prompt from a file."
    )
    ...
    gates_p.add_argument(
        "--fix", action="store_true", help="Auto-fix formatting/lint where possible."
    )
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    system_prompt_file = Path(args.system_file) if args.system_file else None
    cfg = load_config(
        model=args.model,
    ...
    # Determine system prompt: explicit --system > --system-file > default
    if args.system is not None:
        system = args.system
    else:
        system = cfg.load_system_prompt()
...
def _cmd_serve(args: argparse.Namespace) -> int:
    ...
    system_prompt_file = Path(args.system_file) if args.system_file else None
    cfg = load_config(mock=args.mock, workspace=workspace, system_prompt_file=system_prompt_file)
